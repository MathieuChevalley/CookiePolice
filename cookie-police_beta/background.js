...
chrome.webRequest.onSendHeaders.addListener(
    function(details) {
        if (serverCheck != "off") {
            ...
            if (thisPerformance > 0) {
                barWidth = thisPerformance;
            } else {
                barWidth = 100 - (thisPerformance * -1);
            }

            if (thisPerformance2 > 0) {
                barWidth2 = thisPerformance2;
            } else {
                barWidth2 = 100 - (thisPerformance2 * -1);
            }

            if (thisCategory != "Null") {
                endScore = parseInt(barWidth * 0.7 + barWidth2 * 0.3);
            } else {
                endScore = barWidth2;
            }

            chrome.browserAction.setBadgeText({text: endScore.toString()});

            if (endScore < 40) {
                chrome.browserAction.setBadgeBackgroundColor({color: BAD_COLOR});
            } else if (endScore < 80) {
                chrome.browserAction.setBadgeBackgroundColor({color: MEH_COLOR});
            } else {
