...
message.onMessage.addListener(function(msg) {
    ...
    if (category == "Null") {
        ...
        if (performance < 0) {
            performanceAnalyis = "This website's CookieWeight is higher than " + (performance * -1).toString() + "% of all the " + category.toLowerCase() + " websites on our database. Keep scrolling to learn more.";
        } else if (performance > 0 && performance <= 100) {
            performanceAnalyis = "This website's CookieWeight is lower than " + (performance).toString() + "% of all the " + category.toLowerCase() + " websites on our database. We think that's a win for you!";
        }
    ...
    if (background.totalScore != 0) {
        ...
        if (performance2 < 0) {
            performanceAnalyis2 = "This website's CookieWeight is higher than " + (performance2 * -1).toString() + "% of all the websites on our database. Keep scrolling to learn more.";
        } else if (performance2 > 0 && performance2 <= 100) {
            performanceAnalyis2 = "This website's CookieWeight is lower than " + (performance2).toString() + "% of all the websites on our database. We think that's a win for you!";
        }
    ...
    var endScore = background.endScore;
    document.getElementById("scoreBox").innerText = parseInt(endScore) + "%";

    if (endScore < 40) {
        document.getElementById("scoreBox").style.color = BAD_COLOR;
        document.getElementById("scoreExplanationBox").style.color = BAD_COLOR;
        document.getElementById("scoreExplanationBox").innerText = "Poor";
    } else if (endScore < 80) {
        document.getElementById("scoreBox").style.color = OKAY_COLOR;
        document.getElementById("scoreExplanationBox").style.color = OKAY_COLOR;
        document.getElementById("scoreExplanationBox").innerText = "Average";
    } else {
        document.getElementById("scoreBox").style.color = GOOD_COLOR;
        document.getElementById("scoreExplanationBox").style.color = GOOD_COLOR;
        document.getElementById("scoreExplanationBox").innerText = "Excellent";
    }
});
